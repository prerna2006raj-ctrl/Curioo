...
function ProgressPage({
  ...
  const timedOut = progress?.timedOut || 0;

  const answered = progress.questionsAnswered || 0;

  const accuracy =
    answered > 0 ? Math.round((progress.correct / answered) * 100) : 0;
  ...
  const safeGoal = Number(dailyGoal) || 3;

  const goalProgress = Math.min(safeGoal, Number(todayTopics) || 0);

  const goalPercent = Math.min(
    100,
    Math.round((goalProgress / safeGoal) * 100),
  );
  ...
  const progressBar = `${Math.min(100, accuracy)}%`;
  ...
  return (
    <>
      <section className="max-w-4xl mx-auto">
        ...
        >
          <p className="text-xs opacity-50 font-display">Learning streak</p>
            ...
            {bestTopic ? (
              <>
                ...
                <p className="font-body text-sm mt-1 opacity-70">
                  {bestTopic.accuracy}% accuracy · {bestTopic.attempts} attempt
                  {bestTopic.attempts !== 1 ? "s" : ""}
                </p>
              </>
            ) : (
          ...
          >
            ...
            {weakestTopic ? (
              <>
                ...
                <p className="font-body text-sm mt-1 opacity-70">
                  {weakestTopic.accuracy}% accuracy · {weakestTopic.attempts}{" "}
                  attempt
                  {weakestTopic.attempts !== 1 ? "s" : ""}
                </p>
              </>
            ) : (
              <p className="font-body text-sm mt-3 opacity-60">
        ...
        <div className="rounded-3xl border border-line/20 dark:border-line-dark/20 bg-panel dark:bg-blueprint-panel p-6 mb-5 mt-5">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              <p className="font-display text-sm">{correct} correct</p>
          ...
          {chartData.length === 0 ? (
            <div className="h-72 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="font-display">No quiz data yet</p>
              ...
              {[...topicStats]
                ...
                .map((item) => (
                  ...
                  >
                    ...
                    <p className="font-body text-xs opacity-50 mt-1">
                      {item.correct} correct out of {item.answered}
                    </p>
                  </div>
                ))}
            </div>
      ...
      {streakView && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-panel dark:bg-blueprint-panel border border-line/20 dark:border-line-dark/20 p-6 shadow-2xl">
            ...
            <StreakCalendar log={log} range={streakView} />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 mt-5">
                <div className="rounded-2xl border border-line/10 p-4">
                  <p className="text-xs opacity-50">Attempts</p>
                ...
                <div className="rounded-2xl border border-line/10 p-4">
                  <p className="text-xs opacity-50">Correct</p>
                  <p className="text-xl font-display font-semibold mt-1">
...
function StreakCalendar({ log, range }) {
  ...
  const learningDays = new Set(
    (log || []).map((item) => getKey(item.timestamp)),
  );
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((date) => {
            const active = learningDays.has(getKey(date));
              ...
              >
                ...
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${
                    active ? "bg-green-500" : "bg-ink/5 dark:bg-white/10"
                  }`}
                >
                  {active && "✓"}
                </div>
  ...
  return (
    <div>
      {/* MONTH NAME */}
      <div className="text-center font-semibold text-lg mb-5">{monthName}</div>

      {/* WEEK DAYS */}
      <div className="grid grid-cols-7 gap-2 mb-2">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div key={day} className="text-center text-xs font-medium opacity-60">
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              className={`aspect-square rounded-full flex items-center justify-center text-sm ${
                active ? "bg-green-500 text-white" : "bg-ink/5 dark:bg-white/10"
              }`}
            >
              {date.getDate()}
            </div>
