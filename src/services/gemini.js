...
export async function getExplanation(topic, tone, mode = "standard") {
  const focus =
    {
      standard: "Give a balanced explanation.",

      simpler:
        "Make the explanation much simpler, using very easy words and short sentences.",

      example: "Explain the idea using a clear everyday example.",

      steps: "Focus strongly on a logical step-by-step explanation.",

      realworld: "Focus on a practical real-world example or situation.",

      analogy: "Explain the idea using one strong, easy-to-understand analogy.",
    }[mode] || "Give a balanced explanation.";

  const prompt = `
Explain "${topic}" for someone who is ${
    tone === "kid" ? "5 years old" : "an engineer"
  }.
...
export async function getLearningLesson(topic, tone) {
  const prompt = `
Create a short interactive mini lesson about "${topic}".

The learner is ${tone === "kid" ? "5 years old" : "an engineer"}.
...
`;
  ...
  const title = raw.match(/Title:\s*(.+)/i)?.[1]?.trim() || topic;
  ...
  const steps = [1, 2, 3, 4].map((number) => {
    const match = raw.match(
      new RegExp(`Step ${number}:\\s*([^|\\n]+)\\s*\\|\\s*([^\\n]+)`, "i"),
    );

    return {
      title: match?.[1]?.trim() || `Step ${number}`,

      text: match?.[2]?.trim() || "Let's understand this part of the topic.",
    };
  });

  const remember = raw.match(/Remember:\s*(.+)/i)?.[1]?.trim() || "";
...
export async function getQuiz(topic, difficulty = "medium") {
  const difficultyInstructions = {
    ...
    medium:
      "Test understanding and application. Require some reasoning but stay accessible.",
...
`;
  ...
  try {
    ...
    console.error("Quiz JSON parsing error:", error, raw);

    throw new Error("Couldn't understand the quiz response");
  }
